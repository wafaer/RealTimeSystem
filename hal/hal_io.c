/* HAL data I/O — pin and parameter registry.
 *
 * Pins and parameters are the externally-visible data points of a HAL
 * component:
 *   - Pins  carry signals between components and may be linked together;
 *           every pin has a hidden dummy signal so unlinked pins still have
 *           a valid storage location.
 *   - Params expose internal state for inspection (RO) or runtime tuning
 *           (RW).
 *
 * This module owns:
 *   - hal_pin_new / hal_pin_newf / hal_pin_newfv: pin creation, sorted into
 *     hal_data->pin_list_ptr by name.
 *   - hal_param_new and the typed wrappers (bit / s32 / float): parameter
 *     creation, sorted into hal_data->param_list_ptr by name.
 *   - alloc_pin_struct / free_pin_struct / alloc_param_struct /
 *     free_param_struct / free_oldname_struct: backing-store management.
 */
